...
static const AVCodecTag flv_video_codec_ids[] = {{AV_CODEC_ID_FLV1, FLV_CODECID_H263},
...
extern int ff_hevc_annexb2mp4_buf(const uint8_t* buf_in, uint8_t** buf_out, int* size, int filter_ps, int* ps_count);

extern int ff_isom_write_hvcc(AVIOContext* pb, const uint8_t* data, int size, int ps_array_completeness);
...
static int get_audio_flags(AVFormatContext* s, AVCodecParameters* par)
{
    ...
    {
        return FLV_CODECID_OPUS | FLV_SAMPLERATE_44100HZ | FLV_SAMPLESSIZE_16BIT | FLV_STEREO;
    }
...
static void write_metadata(AVFormatContext* s, unsigned int ts)
{
    ...
    metadata_count     = 4 * !!flv->video_par + 5 * !!flv->audio_par + 1 * !!flv->data_par;
    ...
    {
        if (!strcmp(tag->key, "width") || !strcmp(tag->key, "height") || !strcmp(tag->key, "videodatarate")
            || !strcmp(tag->key, "framerate") || !strcmp(tag->key, "videocodecid") || !strcmp(tag->key, "audiodatarate")
            || !strcmp(tag->key, "audiosamplerate") || !strcmp(tag->key, "audiosamplesize")
            || !strcmp(tag->key, "stereo") || !strcmp(tag->key, "audiocodecid") || !strcmp(tag->key, "duration")
            || !strcmp(tag->key, "onMetaData") || !strcmp(tag->key, "datasize") || !strcmp(tag->key, "lasttimestamp")
            || !strcmp(tag->key, "totalframes") || !strcmp(tag->key, "hasAudio") || !strcmp(tag->key, "hasVideo")
            || !strcmp(tag->key, "hasCuePoints") || !strcmp(tag->key, "hasMetadata")
...
static int unsupported_codec(AVFormatContext* s, const char* type, int codec_id)
{
    ...
    av_log(s, AV_LOG_ERROR, "%s codec %s not compatible with flv\n", type, desc ? desc->name : "unknown");
...
static int get_codec_fourcc(int codec_id, int* fourcc_int)
{
    ...
    *fourcc_int = ((int)fourcc[0]) << 24 | ((int)fourcc[1]) << 16 | ((int)fourcc[2]) << 8 | fourcc[3];
    return 0;
}
...
static void flv_write_codec_header(AVFormatContext* s, AVCodecParameters* par, int64_t ts)
{
    ...
    if (par->codec_id == AV_CODEC_ID_AAC || par->codec_id == AV_CODEC_ID_H264 || par->codec_id == AV_CODEC_ID_MPEG4
        || par->codec_id == AV_CODEC_ID_HEVC || par->codec_id == AV_CODEC_ID_OPUS || par->codec_id == AV_CODEC_ID_VP8
        || par->codec_id == AV_CODEC_ID_VP9)
    {
        int64_t pos;
        avio_w8(pb, par->codec_type == AVMEDIA_TYPE_VIDEO ? FLV_TAG_TYPE_VIDEO : FLV_TAG_TYPE_AUDIO);
        ...
        if (par->codec_id == AV_CODEC_ID_AAC)
        {
            ...
            if (!par->extradata_size && (flv->flags & FLV_AAC_SEQ_HEADER_DETECT))
            {
                ...
                int           channels
                    = flv->audio_par->ch_layout.nb_channels - (flv->audio_par->ch_layout.nb_channels == 8 ? 1 : 0);
                uint8_t data[2];

                for (samplerate_index = 0; samplerate_index < 16; samplerate_index++)
                    if (flv->audio_par->sample_rate == ff_mpeg4audio_sample_rates[samplerate_index])
                        break;
...
static int flv_init(struct AVFormatContext* s)
{
    ...
    for (i = 0; i < s->nb_streams; i++)
    {
        ...
        case AVMEDIA_TYPE_VIDEO:
            if (s->streams[i]->avg_frame_rate.den && s->streams[i]->avg_frame_rate.num)
            {
                flv->framerate = av_q2d(s->streams[i]->avg_frame_rate);
            }
            if (flv->video_par)
            {
                av_log(s, AV_LOG_ERROR, "at most one video stream is supported in flv\n");
                return AVERROR(EINVAL);
            }
            flv->video_par = par;
            ...
            if (par->codec_id == AV_CODEC_ID_MPEG4 || par->codec_id == AV_CODEC_ID_H263)
            {
                ...
                if (error)
                {
                    av_log(s, AV_LOG_ERROR, "use vstrict=-1 / -strict -1 to use it anyway.\n");
                    return AVERROR(EINVAL);
                }
            }
            else if (par->codec_id == AV_CODEC_ID_VP6)
            {
                av_log(s, AV_LOG_WARNING,
                       "Muxing VP6 in flv will produce flipped video on "
                       "playback.\n");
            }
            break;
        case AVMEDIA_TYPE_AUDIO:
            if (flv->audio_par)
            {
                av_log(s, AV_LOG_ERROR, "at most one audio stream is supported in flv\n");
                return AVERROR(EINVAL);
            }
            flv->audio_par = par;
            ...
            if (par->codec_id == AV_CODEC_ID_PCM_S16BE)
                av_log(s, AV_LOG_WARNING,
                       "16-bit big-endian audio in flv is valid but most "
                       "likely unplayable (hardware dependent); use s16le\n");
            break;
        ...
        case AVMEDIA_TYPE_SUBTITLE:
            ...
            {
                av_log(s, AV_LOG_ERROR,
                       "Subtitle codec '%s' for stream %d is not compatible "
                       "with FLV\n",
...
static int flv_write_header(AVFormatContext* s)
{
    ...
    avio_write(pb, "FLV", 3);
    avio_w8(pb, 1);
    avio_w8(pb, FLV_HEADER_FLAG_HASAUDIO * !!flv->audio_par + FLV_HEADER_FLAG_HASVIDEO * !!flv->video_par);
    avio_wb32(pb, 9);
    avio_wb32(pb, 0);
        ...
        for (i = 0; i < s->nb_streams; i++)
        {
            ...
            else
            {
                if (par->codec_type == AVMEDIA_TYPE_VIDEO
                    && (par->codec_id == AV_CODEC_ID_H265 || par->codec_id == AV_CODEC_ID_H264
...
static int flv_write_packet(AVFormatContext* s, AVPacket* pkt)
{
    ...
    if (par->codec_id == AV_CODEC_ID_VP6F || par->codec_id == AV_CODEC_ID_VP6A || par->codec_id == AV_CODEC_ID_VP6
        || par->codec_id == AV_CODEC_ID_AAC || par->codec_id == AV_CODEC_ID_OPUS)
        flags_size = 2;
    ...
    if (par->codec_id == AV_CODEC_ID_AAC || par->codec_id == AV_CODEC_ID_H264 || par->codec_id == AV_CODEC_ID_MPEG4
        || par->codec_id == AV_CODEC_ID_HEVC || par->codec_id == AV_CODEC_ID_VP8 || par->codec_id == AV_CODEC_ID_VP9
    ...
    if (par->codec_id == AV_CODEC_ID_H264 || par->codec_id == AV_CODEC_ID_MPEG4 || par->codec_id == AV_CODEC_ID_H265
    ...
    else if (par->codec_id == AV_CODEC_ID_AAC && pkt->size > 2 && (AV_RB16(pkt->data) & 0xfff0) == 0xfff0)
    {
    ...
    if (par->codec_type == AVMEDIA_TYPE_DATA || par->codec_type == AVMEDIA_TYPE_SUBTITLE)
    {
        int     data_size;
    ...
    else
    {
        ...
        if (par->codec_id == AV_CODEC_ID_VP6F || par->codec_id == AV_CODEC_ID_VP6A)
        {
            if (par->extradata_size)
                ...
                avio_w8(pb, ((FFALIGN(par->width, 16) - par->width) << 4) | (FFALIGN(par->height, 16) - par->height));
        }
...
static const AVOption options[] = {
    ...
    {"add_keyframe_index",
     ...
     "flvflags"},
    {"ext_header",
     "enable extern header",
     0,
     AV_OPT_TYPE_CONST,
     {.i64 = FLV_EXTERN_HEADER},
     INT_MIN,
     INT_MAX,
     AV_OPT_FLAG_ENCODING_PARAM,
     "flvflags"},
...
const FFOutputFormat ff_flv_muxer = {
    ...
    .p.codec_tag     = (const AVCodecTag* const[]){flv_video_codec_ids, flv_audio_codec_ids, 0},
