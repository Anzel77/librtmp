...
void *rtmp_flv_sender_alloc(const char *url) // return handle
{
    RTMP_Log(RTMP_LOGINFO, "=== rtmp_flv_sender_alloc ===");
    ...
    rtmp = RTMP_Alloc();
    RTMP_Init(rtmp);
    rtmp->Link.timeout = 5; // It takes a few secs to wait the connection of
                            // network, before return timeout.
    ...
    rtmp_flv = calloc(1, sizeof(RTMP_FLV));
    rtmp_flv->rtmp = rtmp;
    return (void *)rtmp_flv;
}

// @brief free rtmp_sender handler
// @param [in] rtmp_sender handler
void rtmp_flv_sender_free(void *handle) {
    RTMP_Log(RTMP_LOGINFO, "=== rtmp_flv_sender_free ===");
...
int rtmp_flv_sender_start_publish(void *handle, uint32_t flag, int64_t ts_us) {
    RTMP_Log(RTMP_LOGINFO, "=== rtmp_flv_sender_start_publish ===");

    RTMP_FLV *rtmp_flv = (RTMP_FLV *)handle;
    RTMP *rtmp;
...
// @param [in] rtmp_sender handler
// @return             : 0: OK; others: FAILED
int rtmp_flv_sender_stop_publish(void *handle) {
    RTMP_Log(RTMP_LOGINFO, "=== rtmp_flv_sender_stop_publish ===");
 ...
 */
static uint8_t *get_adts(uint32_t *len, uint8_t **offset, uint8_t *start,
                         uint32_t total) {
    uint8_t *p = *offset;
...
int rtmp_flv_sender_write_aac_frame(void *handle, uint8_t *data, int size,
                                uint64_t dts_us, uint32_t start_time) {
    int ret;
    RTMP_FLV *rtmp_flv = (RTMP_FLV *)handle;
    RTMP *rtmp;
    ...
    while (1) {
        ...
        offset = 0;
        audio_frame =
            get_adts(&adts_len, &audio_buf_offset, audio_buf, audio_buf_len);
        if (audio_frame == NULL)
            break;
        if (rtmp_flv->audio_config_ok == 0) {
            rtmp_flv->config = gen_config(audio_frame);
            ...
            // uint8_t header = gen_audio_tag_header(rtmp_flv->config);
            // RTMP_Log(RTMP_LOGINFO, "rtmp_sender_write_aac_audio_frame--config
            // = 0: header=%X\n", header);

            output[offset++] =
                gen_audio_tag_header(rtmp_flv->config); // sound format aac
            output[offset++] = 0x00;                      // aac sequence header

            // flv AudioTagBody --AudioSpecificConfig
            uint8_t audio_object_type =
                rtmp_flv->config.audio_object_type + 1;
            output[offset++] = (audio_object_type << 3) |
                               (rtmp_flv->config.sample_frequency_index >> 1);
            output[offset++] =
                ((rtmp_flv->config.sample_frequency_index & 0x01) << 7) |
                (rtmp_flv->config.channel_configuration << 3);
            ...
            */
            ...
            body_len =
                2 + adts_len -
                AAC_ADTS_HEADER_SIZE; // remove adts header + AudioTagHeader
            output_len = body_len + RTMP_MESSAGE_HEADER_LEN + FLV_PRE_TAG_LEN;
            output = malloc(output_len);
            // Message Header
            ...
            // flv AudioTagHeader
            //  uint8_t header = gen_audio_tag_header(rtmp_flv->config);
            //  printf("rtmp_sender_write_aac_audio_frame--config = 1:
            //  header=%X\n", header);
            ...
            memcpy(output + offset, audio_frame + AAC_ADTS_HEADER_SIZE,
                   (adts_len -
                    AAC_ADTS_HEADER_SIZE)); // H264 sequence parameter set
...
int rtmp_flv_sender_write_avc_frame(void *handle, uint8_t *data, int size,
                                uint64_t dts_us, int key) {
    int ret;
    ...
    while (1) {
        ...
        if ((nal[0] & 0x1f) == 0x07) {
            ...
            // flv VideoTagHeader
            output[offset++] =
                (FLV_FRAME_KEY & 0xf0) | (FLV_CODECID_H264 & 0x0f); // key frame
            ...
            output[offset++] = 0xff; // reserved + length size minus one, (0xFC
                                     // | length size minus one)
            output[offset++] = 0xe1; // 0xE0 | num of SPS NALUs (usually 1)
            output[offset++] = (uint8_t)(
                nal_len >> 8); // sequence parameter set length high 8 bits
            output[offset++] =
                (uint8_t)(nal_len); // sequence parameter set  length low 8 bits
            memcpy(output + offset, nal,
                   nal_len); // H264 sequence parameter set
            offset += nal_len;
            output[offset++] = 0x01; // num of PPS NALUs (usually 1)
            output[offset++] = (uint8_t)(
                nal_len_pps >> 8); // picture parameter set length high 8 bits
            output[offset++] = (uint8_t)(
                nal_len_pps); // picture parameter set length low 8 bits
            memcpy(output + offset, nal_pps,
                   nal_len_pps); // H264 picture parameter set
        ...
        if ((nal[0] & 0x1f) == 0x01) {
            ...
            // flv VideoTagHeader
            output[offset++] = (FLV_FRAME_INTER & 0xf0) |
                               (FLV_CODECID_H264 & 0x0f); // key frame
            output[offset++] = 0x01;                      // avc NALU unit
            output[offset++] = 0x00; // composit time ??????????
            output[offset++] = 0x00; // composit time
            output[offset++] = 0x00; // composit time
 ...
 *
 * @param handle RTMP_FLV handle
 * @return int status value, 1: connected, 0: not connected
 */
int rtmp_flv_sender_connect_state(void *handle) {
    RTMP_FLV *rtmp_flv;
    RTMP *rtmp;
...
static void bytestream_put(char *const stream, const uint32_t value,
                           uint8_t length, uint32_t *offset) {
    /* big-endian */
 ...
 * @return int
 */
int rtmp_sender_write_opus_frame(void *handle, uint8_t *data, int size,
                                 uint64_t dts_us, uint32_t start_time) {
    int ret;
    RTMP_FLV *rtmp_flv = (RTMP_FLV *)handle;
    RTMP *rtmp;
    ...
    while (1) {
        ...
        if (rtmp_flv->audio_config_ok == 0) {
            body_len =
                2 + OPUS_HEADER_SIZE; // FlvAudioTagHeader + OpusHeaderSize
            output_len = body_len + RTMP_MESSAGE_HEADER_LEN + FLV_PRE_TAG_LEN;
            output = malloc(output_len);
            // Message Header
            ...
            // flv AudioTagHeader
            // output[offset++] = 0xDF;
            output[offset++] = FLV_CODECID_OPUS | FLV_SAMPLERATE_44100HZ |
            ...
            // opus_ts); no need to set pre_tag_size
            /*
            uint32_t fff = body_len + RTMP_MESSAGE_HEADER_LEN;
            output[offset++] = (uint8_t)(fff >> 24); //data len
            output[offset++] = (uint8_t)(fff >> 16); //data len
            ...
            */
            ...
            body_len =
                2 + opus_frame_len; // remove adts header + AudioTagHeader
            output_len = body_len + RTMP_MESSAGE_HEADER_LEN + FLV_PRE_TAG_LEN;
            output = malloc(output_len);
            // Message Header
            output[offset++] = RTMP_PACKET_TYPE_AUDIO;    // TagType
...
int rtmp_sender_write_hevc_frame(void *handle, uint8_t *data, int size,
                                 uint64_t dts_us, int key) {
    int ret;
    uint8_t *buf; // video data
    uint8_t *buf_offset;
    ...
    while (1) {
        ...
        // VPS(Video parameter set), SPS(Sequence Parameter Set) and PPS(Picture
        // Parameter Set)
        if (((nal[0] >> 1) & 0x3f) == 0x20) {
            ...
            // flv VideoTagHeader
            output[offset++] =
                (FLV_FRAME_KEY & 0xf0) | (FLV_CODECID_HEVC & 0x0f); // key frame
            output[offset++] = 0x00; // avc sequence header
            output[offset++] = 0x00; // composite time
            output[offset++] = 0x00; // composite time
            output[offset++] = 0x00; // composite time
        ...
        if (((nal[0] >> 1) & 0x3f) == 0x01) {
            ...
            // flv VideoTagHeader
            output[offset++] = (FLV_FRAME_INTER & 0xf0) |
                               (FLV_CODECID_HEVC & 0x0f); // key frame
            output[offset++] = 0x01;                      // avc NALU unit
            output[offset++] = 0x00; // composit time ??????????
            output[offset++] = 0x00; // composit time
            output[offset++] = 0x00; // composit time
